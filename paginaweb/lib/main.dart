...
// ===================== SHELL PRINCIPAL =====================
// Páginas: 1=Inicio, 2=Login, 3=Opciones, 4=Formulario, 5=Resultado
...
class _AppShellState extends State<AppShell> {
  ...
  // Si el usuario quiere crear plan, verifica si está logueado
  void irACrearPlan() {
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      irA(3); // Ya está logueado → va directo a opciones
    } else {
      irA(2); // No está logueado → va al login primero
    }
  }
...
Genera:
- Menú según el periodo indicado, de forma detallada (desayuno, comida y cena)
...
Haz que el output sea lo más sencillo posible y fácil de entender.
''';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: IndexedStack(
        ...
        children: [
          PaginaInicio(onCrearPlan: irACrearPlan),
          LoginPage(onIr: irA),
          ...
          children: [
            ...
            RichText(
              text: const TextSpan(
                ...
                children: [
                  TextSpan(
                      text: 'Byte', style: TextStyle(color: Colors.white)),
                  TextSpan(text: '&', style: TextStyle(color: naranja)),
                  TextSpan(
                      text: 'Bite', style: TextStyle(color: Colors.white)),
                ],
              ),
            ),
          ],
...
class LoginPage extends StatefulWidget {
  ...
  const LoginPage({super.key, required this.onIr});
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> login() async {
    ...
    try {
      ...
      widget.onIr(3); // Después del login → opciones
  ...
  Future<void> registrar() async {
    ...
    try {
      ...
      widget.onIr(3); // Después del registro → opciones
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    RichText(
                      text: const TextSpan(
                        ...
                        children: [
                          TextSpan(
                              text: 'Byte',
                              style: TextStyle(color: verdeOscuro)),
                          TextSpan(
                              text: '&', style: TextStyle(color: naranja)),
                          TextSpan(
                              text: 'Bite',
                              style: TextStyle(color: verdeOscuro)),
                        ],
                    ...
                    Text(
                      mostrarRegistro
                          ? 'Regístrate para empezar a generar tu plan'
                          : 'Inicia sesión para crear tu plan alimentario',
...
class PaginaInicio extends StatelessWidget {
  final VoidCallback onCrearPlan;
  const PaginaInicio({super.key, required this.onCrearPlan});
            ...
            child: Column(
              children: [
                ...
                const Text(
                  ...
                  textAlign: TextAlign.center,
                  style: TextStyle(
                      color: Color(0xFF666666), fontSize: 16, height: 1.7),
                ),
                const SizedBox(height: 12),
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                ...
                const SizedBox(height: 28),
                ElevatedButton.icon(
                  onPressed: onCrearPlan,
                  icon: const Text('Crear mi plan gratuito',
                      style: TextStyle(
                ...
                GridView.count(
                  ...
                  children: const [
                    _CardFeature(
                        ...
                        titulo: 'Respeta tu presupuesto',
                        desc:
                            'Ingresa cuánto tienes disponible al mes, quincena o semana. La IA ajusta cada receta para no gastar de más.'),
                    _CardFeature(
                        icono: Icons.family_restroom,
                        titulo: 'Para toda tu familia',
                        desc:
                            'Personaliza el plan para cada integrante. Niños, adultos mayores, personas con diabetes, anemia o alergias.'),
                    _CardFeature(
                        icono: Icons.shopping_cart,
                        titulo: 'Lista de compras lista',
                        desc:
                            'Genera tu lista de súper con productos disponibles en Bodega Aurrerá, Walmart, Chedraui o tu mercado local.'),
                    _CardFeature(
                        icono: Icons.restaurant,
                        titulo: 'Recetas fáciles y rápidas',
                        desc:
                            'Instrucciones paso a paso sin ingredientes raros. Comida real, mexicana, nutritiva y accesible para todos.'),
                  ],
                ),
              ],
            ),
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                ElevatedButton.icon(
                  onPressed: onCrearPlan,
                  icon: const Text('Crear mi plan ahora',
                      style: TextStyle(
  ...
  Widget _buildLogo() {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        const Icon(Icons.eco, color: verdeOscuro, size: 40),
        ...
        RichText(
          text: const TextSpan(
            ...
            children: [
              TextSpan(
                  text: 'Byte', style: TextStyle(color: verdeOscuro)),
              TextSpan(text: '&', style: TextStyle(color: naranja)),
              TextSpan(
                  text: 'Bite', style: TextStyle(color: verdeOscuro)),
            ],
          ),
        ),
      ],
  ...
  Widget _buildStats() {
    return Wrap(
      ...
      children: const [
        _StatItem(
            numero: '52M',
            etiqueta: 'mexicanos en pobreza alimentaria'),
        _StatItem(
            numero: '25%',
            etiqueta: 'hogares con inseguridad alimentaria'),
        _StatItem(
            numero: '70%',
            etiqueta: 'adultos con sobrepeso u obesidad'),
...
class _CardFeature extends StatelessWidget {
  ...
  const _CardFeature(
      {required this.icono, required this.titulo, required this.desc});
  ...
  void _usarPlantilla(String clave) {
    final plantillas = {
      ...
      'foraneo': {
        ...
        'objetivo': 'Más energía en el día',
        'extras': 'Sin mucho tiempo para cocinar',
        'tiendas': ['bodega', 'oxxo'],
      },
      'adultomayor': {
        'presupuesto': '1500',
      ...
      'diabetes': {
        ...
        'objetivo': 'Bajar de peso saludablemente',
        'extras': 'Comida baja en carbohidratos',
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                  'Crea un plan personalizado o elige una plantilla ya lista.',
                  style:
              ...
              Row(
                children: [
                  Expanded(
                    ...
                    desc:
                        'Personaliza todo: presupuesto, número de personas, condiciones de salud, objetivos y tiendas preferidas.',
                    onTap: () => onIr(4),
                  )),
                  ...
                  Expanded(
                    ...
                    desc:
                        'Elige un plan prediseñado para situaciones comunes y empieza en segundos.',
              ...
              GridView.count(
                ...
                children: [
                  _CardPlantilla(
                      icono: Icons.family_restroom,
                      titulo: 'Mamá con 3 hijos',
                      subtitulo: '\$2,000 / mes · 4 personas',
                      onTap: () => _usarPlantilla('mama3hijos')),
                  _CardPlantilla(
                      icono: Icons.school,
                      titulo: 'Estudiante foráneo',
                      subtitulo: '\$1,200 / mes · 1 persona',
                      onTap: () => _usarPlantilla('foraneo')),
                  _CardPlantilla(
                      icono: Icons.elderly,
                      titulo: 'Adulto mayor',
                      subtitulo: '\$1,500 / mes · 2 personas',
                      onTap: () => _usarPlantilla('adultomayor')),
                  _CardPlantilla(
                      icono: Icons.medication,
                      titulo: 'Con diabetes',
                      subtitulo: '\$1,800 / mes · 1 persona',
                      onTap: () => _usarPlantilla('diabetes')),
                ],
              ),
            ],
          ),
      ...
      child: MouseRegion(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
                color:
                    hover ? verdeOscuro : const Color(0xFFdddddd),
                width: 2),
            borderRadius: BorderRadius.circular(20),
            boxShadow: hover
                ? [
                    BoxShadow(
                        color: verdeOscuro.withOpacity(0.12),
                        blurRadius: 20,
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child:
                    Icon(widget.icono, color: verdeOscuro, size: 30),
              ),
              const SizedBox(height: 14),
              Text(widget.titulo,
                  style: const TextStyle(
...
class _PaginaFormularioState extends State<PaginaFormulario> {
  ...
  void _siguientePaso(int siguiente) {
    if (siguiente == 2) {
      if (presupuestoCtrl.text.isEmpty ||
          double.tryParse(presupuestoCtrl.text) == null ||
          double.parse(presupuestoCtrl.text) <= 0) {
        setState(() =>
            error = 'Por favor ingresa un presupuesto mayor a \$0.');
        return;
      }
      if (periodoSeleccionado.isEmpty) {
        setState(() => error =
            'Por favor selecciona el período de tu presupuesto.');
        return;
      }
  ...
  Widget _buildPaso1() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _encabezadoPaso(Icons.payments, 'Tu presupuesto',
            '¿Cuánto tienes disponible para la comida de tu familia?'),
        ...
        _campoForm('¿Cada cuánto es ese presupuesto?',
            child: GridView.count(
              ...
              children: [
                _opcionPeriodo(
                    'Mensual', 'Cada mes', Icons.calendar_month, 'mensual'),
                _opcionPeriodo('Quincenal', 'Cada 15 días',
                    Icons.date_range, 'quincenal'),
                _opcionPeriodo(
                    'Semanal', 'Cada semana', Icons.view_week, 'semanal'),
                _opcionPeriodo('Diario', 'Por día', Icons.today, 'diario'),
              ],
            )),
        _navForm(
          atras: TextButton.icon(
              onPressed: () => widget.onIr(3),
  ...
  Widget _buildPaso2() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _encabezadoPaso(Icons.family_restroom, 'Tu familia',
            'Cuéntanos sobre las personas para las que planearás la comida.'),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              border: Border.all(
                  color: const Color(0xFFa5d6a7), width: 2),
              borderRadius: BorderRadius.circular(11),
            ),
            child: const Row(
              mainAxisAlignment: MainAxisAlignment.center,
  ...
  Widget _buildPaso3() {
    final tiendas = [
      ...
      {
        'key': 'oxxo',
        'nombre': 'OXXO / tiendita',
        'icono': Icons.night_shelter
      },
    ];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _encabezadoPaso(Icons.location_on, 'Dónde compras',
            'Selecciona las tiendas que tienes cerca.'),
        _campoForm('Tu ciudad o colonia',
            child: TextField(
                controller: ciudadCtrl,
                decoration:
                    _inputDec('Ej: Puebla, Col. Centro...'))),
        _campoForm('Tiendas donde acostumbras comprar',
            child: GridView.count(
              ...
              children: tiendas.map((t) {
                final seleccionado =
                    tiendasSeleccionadas.contains(t['key']);
                return GestureDetector(
                  onTap: () => setState(() {
                    if (seleccionado)
                      tiendasSeleccionadas.remove(t['key']);
                    else
                      tiendasSeleccionadas.add(t['key'] as String);
                  }),
                  child: AnimatedContainer(
  ...
  Widget _buildPaso4() {
    final objetivos = [
      ...
      {
        'value': 'Bajar de peso saludablemente',
        'label': 'Bajar de peso saludablemente'
      },
      {
        'value': 'Subir de peso / ganar masa',
        'label': 'Subir de peso / ganar masa'
      },
      {
        'value': 'Más energía en el día',
        'label': 'Más energía en el día'
      },
      {
        'value': 'Mejorar salud digestiva',
        'label': 'Mejorar salud digestiva'
      },
    ];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _encabezadoPaso(Icons.flag, '¿Algún objetivo?',
            'Si tienes alguna meta específica, la IA la tomará en cuenta.'),
        _campoForm('Objetivo principal',
            child: DropdownButtonFormField<String>(
              ...
              items: objetivos
                  ...
                  .toList(),
              onChanged: (v) =>
                  setState(() => objetivoSeleccionado = v ?? ''),
            )),
        _campoForm('Algo extra que quieras que sepa la IA',
            child: TextField(
              ...
              maxLines: 3,
              decoration: _inputDec(
                  'Ej: No me gusta el picante, prefiero comida rápida...'),
            )),
        const SizedBox(height: 12),
        Row(
          children: [
            ...
            ElevatedButton.icon(
              ...
              label: const Text('Generar mi plan',
                  style: TextStyle(fontWeight: FontWeight.bold)),
              style: ElevatedButton.styleFrom(
      ...
      child: Column(
        children: [
          ...
          Row(children: [
            Expanded(
                child: _miniCampo(
                    'Nombre (opcional)', p['nombre']!, 'Ana')),
            const SizedBox(width: 10),
            Expanded(
                child: _miniCampo('Edad', p['edad']!, '35',
                    isNum: true)),
          ]),
          const SizedBox(height: 10),
          Row(children: [
            Expanded(
                child: _miniDropdown(
                    ...
                    (v) =>
                        setState(() => personas[i].genero = v ?? ''))),
            const SizedBox(width: 10),
            Expanded(
                child: _miniCampo('Peso (kg)', p['peso']!, '60',
                    isNum: true)),
          ]),
          const SizedBox(height: 10),
          Row(children: [
            Expanded(
                child: _miniCampo('Estatura (cm)', p['altura']!, '160',
                    isNum: true)),
            const SizedBox(width: 10),
            Expanded(
                child: _miniDropdown(
                    'Condición de salud',
                    personas[i].condicion,
                    [
                      '',
                      'Ninguna',
                      'Anemia',
                    ...
                    (v) => setState(
                        () => personas[i].condicion = v ?? ''))),
          ]),
          const SizedBox(height: 10),
          TextField(
            controller: p['alergias'],
            decoration:
                _inputDec('Alergias o alimentos que no le gustan')
                    .copyWith(
  ...
  Widget _miniCampo(String label, TextEditingController ctrl,
      String hint,
      {bool isNum = false}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _miniDropdown(String label, String valor,
      List<String> opciones, Function(String?) onChanged) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label,
  ...
  Widget _opcionPeriodo(
      String nombre, String desc, IconData icono, String valor) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: sel ? verdeOscuro : const Color(0xFFdddddd),
              width: 2),
          borderRadius: BorderRadius.circular(11),
        ),
        child: Column(
  ...
  Widget _encabezadoPaso(
      IconData icono, String titulo, String subtitulo) {
    return Padding(
  ...
  Widget _navForm(
      {required Widget atras, required VoidCallback siguiente}) {
    return Padding(
    ...
    return InputDecoration(
      ...
      border: OutlineInputBorder(
          ...
          borderSide: const BorderSide(
              color: Color(0xFFdddddd), width: 2)),
      ...
      focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(10),
          borderSide:
              const BorderSide(color: verdeOscuro, width: 2)),
      filled: true,
                ...
                Row(
                  children: [
                    Expanded(
                        child: _Chip(
                            valor:
                                '\$${int.tryParse(datos.presupuesto) ?? datos.presupuesto}',
                            etiq: periodoLabel[datos.periodo] ??
                                datos.periodo)),
                    const SizedBox(width: 12),
                    Expanded(
                        child: _Chip(
                            valor: '${datos.personas.length}',
                            etiq: 'Personas')),
                    const SizedBox(width: 12),
                    Expanded(
                        child: _Chip(
                            valor: datos.tiendas.isNotEmpty
